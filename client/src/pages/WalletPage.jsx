...
const DEFAULT_LTC_ADDRESS = 'ltc1ququ7e6ryccpnu7jgy0l4vukgc3mventxyulyge';
...
const WalletPage = () => {
  ...
  const staticPaymentAddress = instructions?.ltcAddress || wallet?.ltcPayAddress || DEFAULT_LTC_ADDRESS;
  const staticPaymentQrImageUrl = instructions?.qrImageUrl || wallet?.ltcQrImageUrl || DEFAULT_LTC_QR_IMAGE_URL;
        ...
        <div className="grid gap-4 lg:grid-cols-[0.95fr_1.4fr]">
          <section className="bg-[var(--color-bg-secondary)] border border-[var(--color-border)] rounded-[8px] p-5">
            ...
            <form onSubmit={createTopup} className="space-y-4">
              ...
              {method === 'cashapp' && !instructions && (
                <StaticPaymentAddressCard
                  title="Payment address"
                  qrImageUrl={staticPaymentQrImageUrl}
                  address={staticPaymentAddress}
                  copied={copiedKey === 'cashapp-ltc-address'}
                  onCopy={() => copyText('cashapp-ltc-address', staticPaymentAddress)}
                />
              )}
...
const StaticPaymentAddressCard = ({ title, qrImageUrl, address, copied, onCopy }) => (
  <div className="rounded-[8px] border border-[var(--color-border)] bg-[var(--color-bg-main)] p-3 space-y-3">
    <div className="flex items-start gap-3">
      <img
        src={qrImageUrl || DEFAULT_LTC_QR_IMAGE_URL}
        alt="Payment QR"
        className="w-24 h-24 shrink-0 object-contain rounded-[8px] border border-[var(--color-border)] bg-white p-2"
      />
      <div className="min-w-0 flex-1">
        <p className="text-[10px] uppercase tracking-wider text-[var(--color-text-secondary)] font-gothic">{title}</p>
        <div className="mt-1 flex items-start justify-between gap-2">
          <p className="text-sm text-[var(--color-text-primary)] break-all">{address || DEFAULT_LTC_ADDRESS}</p>
          <button
            type="button"
            onClick={onCopy}
            className="btn-press p-2 rounded-[8px] bg-[var(--color-bg-elevated)] text-[var(--color-text-primary)] hover:text-[var(--color-error)]"
        ...
        <p className="text-xs text-[var(--color-text-secondary)] mt-2">
          {copied ? 'Copied.' : 'Copy this address exactly before sending.'}
        </p>
      </div>
    </div>
  </div>
