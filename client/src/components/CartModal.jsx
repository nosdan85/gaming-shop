...
const CartModal = () => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      setCouponError('');
      setIsCartOpen(false);
      window.location.href = `/pay?orderId=${encodeURIComponent(orderId)}`;
    } catch (err) {
      ...
      <div className="relative bg-[var(--color-bg-main)] w-full h-full md:h-[85vh] md:max-w-4xl md:rounded-[10px] shadow-[rgba(17,24,39,0.12)_0px_24px_60px,rgba(17,24,39,0.06)_0px_8px_24px] flex flex-col md:flex-row overflow-hidden border border-[var(--color-border)] animate-pop-in">
            ...
            {cart.length === 0 ? (
              ...
              cartRows.map(({ item, pricing, deliveredLabel }) => (
                <div key={item._id} className="flex gap-3 md:gap-4 p-3 md:p-4 rounded-[8px] bg-[var(--color-bg-secondary)] border border-[var(--color-border)]">
                    <img
                      src={getProductImageUrl(String(item.image || ''))}
        ...
        <div className="w-full md:w-2/5 bg-[var(--color-bg-secondary)] p-4 md:p-6 flex flex-col min-h-0 h-2/5 md:h-full relative overflow-y-auto">
          ...
          <div className="bg-[var(--color-bg-main)] p-4 rounded-[8px] border border-[var(--color-border)] mb-4">
            {user && user.discordId ? (
              ...
              <div className="text-center space-y-3">
                <p className="text-[var(--color-text-secondary)] text-xs font-serif">Discord login is optional now. You will link Discord after payment to create your ticket.</p>
                {isMobile ? (
          ...
          <div className="flex-1 overflow-y-auto mb-4 pr-1">
            <h3 className="text-[var(--color-text-secondary)] text-xs uppercase font-gothic mb-3 tracking-wider">Checkout flow</h3>
            <div className="space-y-3">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-[var(--color-text-primary)] text-sm font-gothic">1. Checkout</p>
                  <p className="text-[var(--color-text-secondary)] text-xs font-serif">Create your order first. Discord and Roblox linking happen after payment.</p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-[var(--color-text-primary)] text-sm font-gothic">2. Pay</p>
                  <p className="text-[var(--color-text-secondary)] text-xs font-serif">Choose PayPal, Cash App, or Litecoin on the payment page.</p>
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-[var(--color-text-primary)] text-sm font-gothic">3. Link accounts</p>
                  <p className="text-[var(--color-text-secondary)] text-xs font-serif">After payment is confirmed, link Discord and Roblox.</p>
                </div>
              </div>
              <div className="flex gap-3">
                <div className="w-8 h-8 rounded-full bg-[var(--color-bg-elevated)] flex items-center justify-center flex-shrink-0 text-[var(--color-success)]">
                ...
                <div>
                  <p className="text-[var(--color-text-primary)] text-sm font-gothic">4. Create ticket</p>
                  <p className="text-[var(--color-text-secondary)] text-xs font-serif">Create your delivery ticket only after payment and account linking.</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <button
            ...
            disabled={cart.length === 0 || isProcessing || hasInvalidCheckoutTotal}
